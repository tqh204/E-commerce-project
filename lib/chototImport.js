...
const isWardLike = (value = '') => /^(p\.?|phuong|phường|x\.?|xa|xã|tt\.?|thi tran|thị trấn|ward)\b/i.test(value);
const isDistrictLike = (value = '') => /^(q\.?|quan|quận|h\.?|huyen|huyện|thi xa|thị xã|tp\.?|thanh pho|thành phố)\b/i.test(value);
...
const parseAreaFromTitle = (title = '') => {
  const match = normalizeWhitespace(title).match(/\btại\s+(.+?)(?:\s+được đăng bởi|\s+duoc dang boi|$)/iu);
  if (!match) {
    return { province: '', district: '', ward: '' };
  }
...
const cleanListingTitle = (title = '') => {
  const cleaned = normalizeWhitespace(
    title
      ...
      .replace(/\s+được đăng bởi.*$/iu, '')
      .replace(/\s+duoc dang boi.*$/i, '')
      .replace(/\s*\|\s*Ch[oợ] T[ốo]t.*$/iu, '')
      .replace(/\s+tại\s+.+$/iu, '')
      .replace(/\s+tai\s+.+$/i, '')
  );
...
const cleanImportedItem = (item) => {
  ...
  const normalizedTitle = normalizeWhitespace(
    title
      ...
      .replace(/\b(cần bán|can ban|chính chủ|chinh chu|bao sang ten|bao sang tên)\b/giu, '')
  ).substring(0, 200);
...
const ensureChototImporterUser = async () => {
  const sellerRole =
    (await Role.findOne({ name: 'seller' })) ||
    (await Role.create({
      name: 'seller',
  ...
  if (!user.roles.some((roleId) => `${roleId}` === `${sellerRole._id}`)) {
