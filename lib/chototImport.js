...
const isHtmlLike = (value = '') => /<[^>]+>/.test(String(value || ''));
...
const stripHtml = (value = '') => {
  ...
  const $ = cheerio.load(`<div id="fragment">${String(value || '')}</div>`);
  return normalizeWhitespace($('#fragment').text());
};

const parseSrcsetUrls = (value = '', baseUrl = '') =>
  String(value || '')
    .split(',')
    .map((part) => normalizeUrl(part.trim().split(/\s+/)[0], baseUrl))
    .filter(Boolean);

const extractHtmlAttribute = (value = '', attribute = '') => {
  const pattern = new RegExp(attribute + String.raw`\s*=\s*["']([^"']+)`, 'i');
  const match = String(value || '').match(pattern);
  return normalizeWhitespace(match?.[1] || '');
};

const extractHtmlUrls = (value = '', attribute = 'src', baseUrl = '') => {
  const pattern = new RegExp(attribute + String.raw`\s*=\s*["']([^"']+)`, 'ig');
...
const collectImageUrls = ($, root, baseUrl = '') => {
  const urls = new Set();
  const nodes = $(root).find('img, source').add($(root).filter('img, source'));
...
const extractMarkupInfo = (value = '', baseUrl = '') => {
  if (!isHtmlLike(value)) {
    return {
      title: '',
      images: [],
  ...
  const $ = cheerio.load(`<div id="fragment">${raw}</div>`);
...
const cleanImportedItem = (item) => {
  const sourceTitleMarkup = normalizeWhitespace(item.title || item.attributes?.sourceTitle || 'Imported from Chotot');
  const sourceDescriptionMarkup = normalizeWhitespace(item.description || sourceTitleMarkup || 'Imported from Chotot');
  const titleMarkupInfo = extractMarkupInfo(sourceTitleMarkup, item.sourceUrl);
  const descriptionMarkupInfo = extractMarkupInfo(sourceDescriptionMarkup, item.sourceUrl);
  const directTitle = extractHtmlAttribute(sourceTitleMarkup, 'alt') || extractHtmlAttribute(sourceDescriptionMarkup, 'alt');
  const sourceTitle = normalizeWhitespace(directTitle || titleMarkupInfo.title || stripHtml(sourceTitleMarkup) || 'Imported from Chotot');
