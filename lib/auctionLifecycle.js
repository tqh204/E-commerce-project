  ...
  if (cancelled) {
    ...
    await Promise.all([
      createNotification({
        ...
        title: 'Đấu giá đã bị hủy',
        message: 'Phiên đấu giá của bạn đã bị hủy.',
      ...
      currentWinningBid
        ? createNotification({
            ...
            title: 'Đấu giá đã bị hủy',
            message: 'Phiên đấu giá bạn tham gia đã bị hủy.',
            type: 'auction_cancelled',
            refType: 'auction',
            refId: String(auction._id),
            metadata: { auctionId: String(auction._id) },
  ...
  if (reserveMet && winningBid) {
    ...
    await Promise.all([
      createNotification({
        ...
        message: `Bạn đã có người thắng đấu giá với giá ${winningBid.amount.toLocaleString('vi-VN')} VND.`,
        type: 'auction_ended',
        refType: 'auction',
        refId: String(auction._id),
      ...
      createNotification({
        ...
        message: `Bạn đã thắng đấu giá với giá ${winningBid.amount.toLocaleString('vi-VN')} VND.`,
        type: 'auction_won',
        refType: 'auction',
        refId: String(auction._id),
        metadata: { auctionId: String(auction._id), orderId: order ? String(order._id) : null },
    ...
    if (winningBid) {
      ...
      await createNotification({
        userId: winningBid.bidder,
        title: 'Đấu giá kết thúc',
        message: 'Phiên đấu giá kết thúc nhưng không có người thắng.',
        type: 'auction_ended',
        refType: 'auction',
        refId: String(auction._id),
        metadata: { auctionId: String(auction._id) },
